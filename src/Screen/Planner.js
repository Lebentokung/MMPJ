                      ...
                      <View key={it.id} style={styles.classRow}>
                        <View style={styles.classRowLeft}>
                          ...
                          <View style={styles.classTagRow}>
                            ...
                            {it.room ? <View style={styles.tag}><Text style={styles.tagText}>📍 {it.room}</Text></View> : null}
                          </View>
                        </View>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={styles.planCard}>
                  ...
                  <View style={styles.actionBtns}>
                    <TouchableOpacity onPress={() => openEditPlan(item)} style={styles.editBtn}>
                      <Ionicons name="pencil" size={14} color={PINK.primary} />
                    </TouchableOpacity>

                    <TouchableOpacity onPress={() => deletePlan(item.id)} style={styles.delBtn}>
                      <Ionicons name="trash" size={14} color="#e05070" />
                    </TouchableOpacity>
                  </View>
...
//       setEditingPlanId(null);
//     } else {
//       saveStudyPlans([...plans, { id: Date.now().toString(), subject, topic, date, time }]);
//     }
...
//   function deletePlan(id) {
//     Alert.alert('ลบแผน', 'ต้องการลบแผนนี้?', [{ text: 'ยกเลิก' }, {
//       text: 'ลบ', style: 'destructive',
//       onPress: () => saveStudyPlans(plans.filter(item => item.id !== id))
//     }]);
//   }

//   function openEditPlan(plan) {
//     setEditingPlanId(plan.id);
//     setSubject(plan.subject); setTopic(plan.topic);
//     setDate(plan.date); setTime(plan.time);
//     setPlanModalVisible(true);
//   }
...
//   React.useEffect(() => {
//     if (plannerAddRequest) { openAdd(); clearPlannerAddRequest(); }
//   }, [plannerAddRequest]);
...
//   function addActivity() {
//     if (!name.trim()) return Alert.alert('กรุณาใส่ชื่อกิจกรรม');
...
//       setEditingId(null);
//     } else {
//       savePlannerActivities([{ id: Date.now().toString(), name, room, day, classDate, classMonth, start, end }, ...activities]);
//     }
//     setModalVisible(false);
//   }
...
//   function openEdit(item) {
//     setEditingId(item.id);
//     setName(item.name || ''); setRoom(item.room || ''); setDay(item.day || 'Mon');
//     setClassDate(item.classDate || ''); setClassMonth(item.classMonth || '');
//     setStart(item.start || '09:00'); setEnd(item.end || '10:00');
//     setModalVisible(true);
//   }

//   function remove(classItem) {
//     Alert.alert('ลบกิจกรรม', 'ต้องการลบกิจกรรมนี้?', [{ text: 'ยกเลิก' }, {
//       text: 'ลบ', style: 'destructive',
//       onPress: () => savePlannerActivities(activities.filter(i => i.id !== classItem.id))
//     }]);
//   }
